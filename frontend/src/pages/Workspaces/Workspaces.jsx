...
const Workspaces = () => {
  ...
  const overviewStats = [
    ...
    },
    {
      ...
      value: summary?.assigned_tasks ?? 0,
      icon: AlertCircle,
  ...
  const WorkspaceCard = ({ workspace, isHovered }) => (
    <NavLink
      ...
      className={`group relative block cursor-pointer overflow-hidden rounded-2xl border bg-white/70 backdrop-blur-sm transition-all duration-200 dark:bg-white/[0.025] ${isHovered
    ...
    >
      ...
      <div className="border-b border-zinc-100 p-5 dark:border-white/[0.05]">
        <div className="flex items-start justify-between gap-3">
          ...
          <span
            className={`shrink-0 whitespace-nowrap rounded-full px-2.5 py-0.5 text-[11px] font-medium ${getRoleBadgeStyle(
              workspace.role
            )}`}
          >
            {workspace.role}
      ...
      <div className="space-y-3.5 p-5">
        ...
        <div className="grid grid-cols-3 gap-2.5">
          ...
          <div className="rounded-xl bg-zinc-50/80 p-2.5 text-center dark:bg-white/[0.03]">
            <p className="mb-0.5 text-[10.5px] font-medium text-zinc-500 dark:text-zinc-400">
              Members
            </p>
        ...
        <div className="flex items-center gap-2 border-t border-zinc-100 pt-3 dark:border-white/[0.05]">
          ...
          <div className="flex -space-x-1.5">
            ...
            {workspace.recent_members?.slice(0, 3).map((member, idx) => (
              <div
                key={`${member.username}-${idx}`}
                className={`flex h-7 w-7 items-center justify-center overflow-hidden rounded-full border-2 border-white text-[9px] font-bold text-white dark:border-[#0B0C10] ${MEMBER_COLORS[idx % MEMBER_COLORS.length]
                  }`}
              >
                {member.avatar ? (
                  <img
            ...
            {workspace.remaining_members_count > 0 && (
              <div className="flex h-7 w-7 items-center justify-center rounded-full border-2 border-white bg-zinc-200 text-[9px] font-bold text-zinc-600 dark:border-[#0B0C10] dark:bg-white/[0.1] dark:text-zinc-300">
                +{workspace.remaining_members_count}
              </div>
            )}
